...
#include <iostream>
#include "timer.h"
//g++ -Wl,--no-as-needed -larmadillo -lmkl_intel_lp64 -lmkl_sequential -lmkl_core -lpthread -lm -ldl inverse.cpp -o inv
...
int main() {
  ...
  float *in_base_mat_arr = new float[2 * kSize * kSize];
  for (size_t i = 0; i < 2 * kSize * kSize; i++) {
    in_base_mat_arr[i] =
